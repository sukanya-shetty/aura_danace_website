...
<section id=menu>
    ...
    <section id="interface">
        ...
        <div class="values">
            ...
            if(isset($_GET['form']) && isset($_GET['category'])) {
                $form = $_GET['form'];
                $category = $_GET['category'];
                $sql="SELECT st.student_id, st.name, st.phone, st.email, st.age, st.address, st.gender, e.enrollment_id, c.course_name FROM enrollments e JOIN students st ON e.student_id = st.student_id JOIN courses c ON e.course_id = c.course_id WHERE c.course_name='$form'";
                $result = $conn->query($sql);
                if($result && $result->num_rows>0){
                    echo"<tbody>";
                    while($row=$result->fetch_assoc()){
                        $id = isset($row['student_id']) ? $row['student_id'] : 'N/A';
                        $name = isset($row['name']) ? $row['name'] : 'N/A';
                        $mno = isset($row['phone']) ? $row['phone'] : 'N/A';
                        $email = isset($row['email']) ? $row['email'] : 'N/A';
                        $age = isset($row['age']) ? $row['age'] : 'N/A';
                        $addr = isset($row['address']) ? $row['address'] : 'N/A';
                        $gender = isset($row['gender']) ? $row['gender'] : 'N/A';
                        $level = isset($row['course_name']) ? $row['course_name'] : 'N/A';
                        echo"<tr>
                         <td>$id</td>
                          <td>$name</td>
                          <td>$mno</td>
                    ...
                    echo "<tbody><tr><td colspan='8'>No enrollments found for this course</td></tr></tbody>";
                }
            }
            echo "  </tbody>
            </table>";
            ...
            <br><br>
            <button style="background: #7B74EC;margin-top:40px;margin-left:500px;width:100px;"><a href="student.php" style="text-decoration:none;color:white;">Back</a></button>
        </div>
    </section>
</body>
</html>