...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Check if event exists or get the event_id for Wedding event
    $eventQuery = "SELECT event_id FROM events WHERE event_name LIKE '%wedding%' OR event_name LIKE '%celebration%'";
    ...
    if($eventResult->num_rows > 0) {
        ...
        // Create Wedding event if it doesn't exist
        $createEvent = "INSERT INTO events(event_name, event_date, location, capacity, price) 
                       VALUES('Wedding Celebration Event', '$preferredDate', '$state', 100, $amount)";
